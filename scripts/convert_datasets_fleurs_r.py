    ...
    for tsv in root.rglob("*.tsv"):
        with tsv.open("r", encoding="utf-8") as f:
            ...
            for row in reader:
                if not row:
                    continue
                _, utt_id, norm_txt, *_ = row
                utt_id=utt_id.replace(".wav", "")
                flac_path = flac_root / tsv.parent.relative_to(root) / "audio" / "dev" / "dev" / f"{utt_id}.flac"
                if flac_path.exists():
                    yield flac_path.relative_to(flac_root), norm_txt
                flac_path = flac_root / tsv.parent.relative_to(root) / "audio" / "train" / "train" / f"{utt_id}.flac"
                if flac_path.exists():
                    yield flac_path.relative_to(flac_root), norm_txt
                flac_path = flac_root / tsv.parent.relative_to(root) / "audio" / "test" / "test" / f"{utt_id}.flac"
                if flac_path.exists():
                    yield flac_path.relative_to(flac_root), norm_txt
...
def main() -> None:
    ...
    ap.add_argument("--root", required=True, help="FLEURS-R の WAV ルート")
