...
def iter_trans_tsv(root: pathlib.Path, flac_root: pathlib.Path) -> Iterator[tuple[pathlib.Path, str]]:
    for tsv in root.rglob("transcripts_utf8.txt"):
        with tsv.open("r", encoding="utf-8") as f:
            ...
            for row in reader:
                if not row:
                    continue
                utt_id, norm_txt = row
                flac_path = flac_root / tsv.parent.relative_to(root) / "wav24kHz16bit" / f"{utt_id}.flac"
                if flac_path.exists():
...
def main() -> None:
    ...
    ap.add_argument("--root", required=True, help="JVS の WAV ルート")
